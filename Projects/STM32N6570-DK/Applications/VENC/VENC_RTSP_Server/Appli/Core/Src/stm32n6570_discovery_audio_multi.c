...
/* Multi channels Configuration */
/*------------------------------*/
/* One mic is present onboard on STM32N6570-DK Boards */
/* Extra mics can be used through Mics extension boards connected to MIC(cn5) connectors*/
/* STM32N6570-DK_REVA Board supports 2 external mics */
/* STM32N6570-DK_REVB Board supports 4 external mics */
/* Same clock is used for all mics => PE2*/
/* Mics 1&2 => PE8  (SITF0)*/
/* Mics 3&4 (REVB Only) => PE12 (SITF5)*/

/* Extension for recording with up to 2 mics on DK_REVA / 4 on DK_REVB */
#define AUDIO_IN_MAX_CHANNELS_NBR          4U

#define AUDIO_MDF1_DATIN5_GPIO_PORT             GPIOE
#define AUDIO_MDF1_DATIN5_GPIO_CLK_ENABLE()     __HAL_RCC_GPIOE_CLK_ENABLE()
#define AUDIO_MDF1_DATIN5_GPIO_PIN              GPIO_PIN_12
#define AUDIO_MDF1_DATIN5_GPIO_AF               GPIO_AF4_MDF1

/* Use one MDF filter instance per channel (from 1 to 4)*/
MDF_HandleTypeDef  haudio_in_mdf_multi[AUDIO_IN_MAX_CHANNELS_NBR];


MDF_Filter_TypeDef   * filter_per_channel[AUDIO_IN_MAX_CHANNELS_NBR] = {MDF1_Filter0, MDF1_Filter1, MDF1_Filter2, MDF1_Filter3};
uint32_t            bitstream_per_channel[AUDIO_IN_MAX_CHANNELS_NBR] = {MDF_BITSTREAM0_RISING, MDF_BITSTREAM0_FALLING, MDF_BITSTREAM5_RISING, MDF_BITSTREAM5_FALLING};
uint32_t        filter_number_per_channel[AUDIO_IN_MAX_CHANNELS_NBR] = {0, 1, 2, 3};
...
VAR_DECLARE_ALIGN4_AT_SECTION(int32_t, Audio_DigMicRecBuff[DEFAULT_AUDIO_IN_BUFFER_SIZE], SECTION_DMA);

/* Linked list structure - placed in non cached memory*/
VAR_DECLARE_ALIGN4_AT_SECTION(DMA_NodeTypeDef, TxNode, SECTION_DMA);
VAR_DECLARE_ALIGN4_AT_SECTION(DMA_NodeTypeDef, RxNode, SECTION_DMA);
VAR_DECLARE_ALIGN4_AT_SECTION(DMA_NodeConfTypeDef, dmaNodeConfig, SECTION_DMA);
...
int32_t BSP_AUDIO_IN_Init(uint32_t Instance, BSP_AUDIO_Init_t *AudioInit)
{
  ...
  else if (AudioInit->ChannelsNbr == 0 || AudioInit->ChannelsNbr > AUDIO_IN_MAX_CHANNELS_NBR )
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  ...
  else
  {
    ...
    {
      /* Set MDF instances */
      haudio_in_mdf_multi[0].Instance = MDF1_Filter0;

      /* Configure MDF clock according to the requested audio frequency */
      if (MX_MDF1_ClockConfig(&haudio_in_mdf_multi[0], AudioInit->SampleRate) != HAL_OK)
      {
        ret = BSP_ERROR_CLOCK_FAILURE;
      }
      else
      {
        ...
        MDF_MspInit(&haudio_in_mdf_multi[0]);
        ...
        if (ret == BSP_ERROR_NONE)
        {
          ...
          uint32_t maxFilterNumber = 0;
          for (uint16_t channel = 0;channel < AudioInit->ChannelsNbr; channel++)
          {
            maxFilterNumber = max(maxFilterNumber, filter_number_per_channel[channel]);
          }
          ...
          for (uint16_t channel = 0;channel < AudioInit->ChannelsNbr; channel++)
          {
            ...
            /* Instantiate filter number and filter input for each channel*/
            haudio_in_mdf_multi[channel].Instance = filter_per_channel[channel];
            haudio_in_mdf_multi[channel].Init.FilterBistream = bitstream_per_channel[channel];
            
            /* Configure interleaved output*/ 
            haudio_in_mdf_multi[channel].Init.CommonParam.InterleavedFilters = maxFilterNumber;
            
            if (MX_MDF1_Init(&haudio_in_mdf_multi[channel], &mxMdfInit) != HAL_OK)
            {
              ret = BSP_ERROR_PERIPH_FAILURE;
            }
#if (USE_HAL_MDF_REGISTER_CALLBACKS == 1)
            if (ret == BSP_ERROR_NONE)
            {
              ...
              if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[channel], HAL_MDF_ACQ_COMPLETE_CB_ID, MDF_AcqCpltCallback) != HAL_OK)
              {
                ret = BSP_ERROR_PERIPH_FAILURE;
              }
              else if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[channel], HAL_MDF_ACQ_HALFCOMPLETE_CB_ID, MDF_AcqHalfCpltCallback)
                       != HAL_OK)
              {
                ret = BSP_ERROR_PERIPH_FAILURE;
              }
              ...
              {
                if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[channel], HAL_MDF_ERROR_CB_ID, MDF_ErrorCallback) != HAL_OK)
                {
                  ret = BSP_ERROR_PERIPH_FAILURE;
                }
              }
          ...
          {
#ifdef DK_REVB
            /* Start SITF5 - which is connected to PE12 = second mics Data line on DK_REVB.*/
            MDF1_Filter5->SITFCR = 0U;
            MDF1_Filter5->SITFCR |=  ((31U << MDF_SITFCR_STH_Pos) | MDF_SITF_NORMAL_SPI_MODE | MDF_SITF_CCK0_SOURCE);
            MDF1_Filter5->SITFCR |= MDF_SITFCR_SITFEN;
#endif
            /* Update audio in context state */
            Audio_In_Ctx[Instance].State = AUDIO_IN_STATE_STOP;
          }
...
int32_t BSP_AUDIO_IN_DeInit(uint32_t Instance)
{
  ...
  else if (Audio_In_Ctx[Instance].State != AUDIO_IN_STATE_RESET)
  {
    ...
    else /* (Instance == 1U) */
    {
      ...
      if (HAL_MDF_DeInit(&haudio_in_mdf_multi[0]) != HAL_OK)
      {
        ret = BSP_ERROR_PERIPH_FAILURE;
      }
      else
      {
#if (USE_HAL_MDF_REGISTER_CALLBACKS == 0)
        MDF_MspDeInit(&haudio_in_mdf_multi[0]);
#endif /* (USE_HAL_MDF_REGISTER_CALLBACKS == 0) */
      }
...
int32_t BSP_AUDIO_IN_Record(uint32_t Instance, uint8_t *pBuf, uint32_t NbrOfBytes)
{
  ...
  else
  {
    ...
    else /* Instance = 1 */
    {
      ...
      /* Start Interleaved channels*/
      for (uint16_t channel = 1;channel <  Audio_In_Ctx[Instance].ChannelsNbr; channel++)
      {
        if (HAL_MDF_AcqStart(&haudio_in_mdf_multi[channel], &Audio_MdfFilterConfig) != HAL_OK)
        {
          ret = BSP_ERROR_PERIPH_FAILURE;
        }
      }
...
int32_t BSP_AUDIO_IN_Stop(uint32_t Instance)
{
  ...
  else
  {
    ...
    else /* Instance = 1 */
    {
      ...
      if (HAL_MDF_AcqStop_DMA(&haudio_in_mdf_multi[0]) != HAL_OK)
      {
        ret = BSP_ERROR_PERIPH_FAILURE;
      }
    }
...
int32_t BSP_AUDIO_IN_SetSampleRate(uint32_t Instance, uint32_t  SampleRate)
{
  ...
  else
  {
    ...
    else /* (Instance == 1U) */
    {
      ...
      if (MX_MDF1_ClockConfig(&haudio_in_mdf_multi[0], SampleRate) != HAL_OK)
      {
        ret = BSP_ERROR_CLOCK_FAILURE;
      }
      /* Re-initialize MDF1 with new sample rate */
      else if (HAL_MDF_DeInit(&haudio_in_mdf_multi[0]) != HAL_OK)
      {
        ret = BSP_ERROR_PERIPH_FAILURE;
      }
      else
      {
        ...
        MDF_MspInit(&haudio_in_mdf_multi[0]);
#endif
        if (MX_MDF1_Init(&haudio_in_mdf_multi[0], &mxMdfInit) != HAL_OK)
        {
          ret = BSP_ERROR_PERIPH_FAILURE;
        }
      }
      ...
      {
        /* Register MDF filter TC, HT and Error callbacks */
        if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[0], HAL_MDF_ACQ_COMPLETE_CB_ID, MDF_AcqCpltCallback) != HAL_OK)
        {
          ret = BSP_ERROR_PERIPH_FAILURE;
        }
        else if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[0], HAL_MDF_ACQ_HALFCOMPLETE_CB_ID, MDF_AcqHalfCpltCallback) != HAL_OK)
        {
          ret = BSP_ERROR_PERIPH_FAILURE;
        }
        else
        {
          if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[0], HAL_MDF_ERROR_CB_ID, MDF_ErrorCallback) != HAL_OK)
          {
            ret = BSP_ERROR_PERIPH_FAILURE;
          }
        }
...
int32_t BSP_AUDIO_IN_RegisterDefaultMspCallbacks(uint32_t Instance)
{
  ...
  else
  {
    ...
    {
      /* Register MspInit/MspDeInit callbacks */
      if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[0], HAL_MDF_MSPINIT_CB_ID, MDF_MspInit) != HAL_OK)
      {
        status = BSP_ERROR_PERIPH_FAILURE;
      }
      else if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[0], HAL_MDF_MSPDEINIT_CB_ID, MDF_MspDeInit) != HAL_OK)
      {
        status = BSP_ERROR_PERIPH_FAILURE;
      }
      else
...
int32_t BSP_AUDIO_IN_RegisterMspCallbacks(uint32_t Instance, BSP_AUDIO_IN_Cb_t *CallBacks)
{
  ...
  else
  {
    ...
    {
      /* Register MspInit/MspDeInit callbacks */
      if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[0], HAL_MDF_MSPINIT_CB_ID, CallBacks->pMspMdfInitCb) != HAL_OK)
      {
        status = BSP_ERROR_PERIPH_FAILURE;
      }
      else if (HAL_MDF_RegisterCallback(&haudio_in_mdf_multi[0], HAL_MDF_MSPDEINIT_CB_ID, CallBacks->pMspMdfDeInitCb) != HAL_OK)
      {
        status = BSP_ERROR_PERIPH_FAILURE;
      }
      else
...
void BSP_AUDIO_IN_IRQHandler(uint32_t Instance, uint32_t InputDevice)
{
  ...
  {
    HAL_DMA_IRQHandler(haudio_in_mdf_multi[0].hdma);
  }
}
...
__weak HAL_StatusTypeDef MX_MDF1_ClockConfig(MDF_HandleTypeDef *hmdf, uint32_t SampleRate)
{
  ...
  // USe HSI for starting immediately after STBY
  RCC_OscInitStruct.PLL4.PLLSource = RCC_PLLSOURCE_HSI; 
  RCC_OscInitStruct.PLL4.PLLFractional = 0;
  //RCC_OscInitStruct.PLL4.PLLM = 6;  /* 48/6 = 8MHz */
...
static void MDF_MspInit(MDF_HandleTypeDef *hmdf)
{
  ...
  /* MDF pins configuration: DATIN0, DATAIN5 and CCK0 pins */
  ...
  /* REVB Board : Init DATIN5 = mics #3 and #4 input*/
  
  AUDIO_MDF1_DATIN5_GPIO_CLK_ENABLE();
  GPIO_InitStruct.Mode      = GPIO_MODE_AF_PP;
  GPIO_InitStruct.Pull      = GPIO_NOPULL;
  GPIO_InitStruct.Speed     = GPIO_SPEED_FREQ_VERY_HIGH;
  GPIO_InitStruct.Alternate = AUDIO_MDF1_DATIN5_GPIO_AF;
  GPIO_InitStruct.Pin       = AUDIO_MDF1_DATIN5_GPIO_PIN;
  HAL_GPIO_Init(AUDIO_MDF1_DATIN5_GPIO_PORT, &GPIO_InitStruct);
